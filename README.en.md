...
- **Console log ingestion**: `tape console` runs an HTTP endpoint that accepts `console.log` etc. from box WebViews / web pages via GET / POST, with CORS support and instant dumps to `console-YYYYMMDD-HHMMSSmmm.log`.
- **App network log ingestion**: `tape app` accepts in-app logs / network events pushed by boxes that cannot use logcat (GET / POST), dumping instantly to `app-YYYYMMDD-HHMMSSmmm.log`; it shares the same ingestion component as `console`.
...
- tape listens on `0.0.0.0` (all interfaces) by default, so LAN devices can reach it via the IP of the machine running tape; for local-only debugging use `--bind 127.0.0.1` to narrow the exposure.
...
- tape listens on `0.0.0.0` in both modes by default, so LAN devices can reach it directly; use `--bind 127.0.0.1` to restrict access to the local machine (also settable in the `[replay]` config table; CLI wins).
...
- `--bind`: listen address for record / replay, default `0.0.0.0` (all interfaces, for LAN devices). Use `127.0.0.1` for local-only debugging; replay can also read it from the `[replay]` config table, with the CLI taking precedence.
...
- `tape logcat`: live Android logcat viewer with level/keyword filtering; each session is automatically saved to `{log-dir}/logcat-YYYYMMDD-HHMMSSmmm.log` (plain text, no color). See [logcat](#logcat-live-android-logs-with-auto-dump) below.
- `tape console`: starts an HTTP endpoint that accepts debug logs pushed from box WebViews / web pages (GET / POST), automatically saved to `{log-dir}/console-YYYYMMDD-HHMMSSmmm.log`. See [console](#console-box-webview-debug-log-ingestion) below.
- `tape app`: starts an HTTP endpoint that accepts in-app logs / network events pushed by boxes that cannot use logcat (GET / POST), automatically saved to `{log-dir}/app-YYYYMMDD-HHMMSSmmm.log`. See [app](#app-box-app-network-log-ingestion) below.
...
```bash
tape logcat                          # uses the first online device; writes ./logs/logcat-YYYYMMDD-HHMMSSmmm.log
...
- **Auto-dump**: every run creates a timestamped `logcat-YYYYMMDD-HHMMSSmmm.log` (local time) with plain text (no ANSI colors); if writing fails, tape degrades to terminal-only output and warns.
- **Dump naming convention**: log sources are distinguished by prefix and share one `--log-dir` — `logcat-` (device logs), `console-` (WebView console pushes), `app-` (box app network logs), all as `{prefix}-YYYYMMDD-HHMMSSmmm.log`.
...
`tape console` runs an HTTP endpoint that accepts debug logs pushed from WebViews / web pages inside the box, with the same **instant dump** experience as `logcat`: `{log-dir}/console-YYYYMMDD-HHMMSSmmm.log`. Responses carry CORS headers, so cross-origin fetch / XHR from web pages works without certificates or a proxy.

```bash
tape console                          # listens on 0.0.0.0:8899; writes ./logs/console-YYYYMMDD-HHMMSSmmm.log
...
`tape app` accepts in-app logs / network events pushed by boxes that **cannot use logcat** (e.g. no adb on the firmware, or the app wants to report directly), dumping instantly to `{log-dir}/app-YYYYMMDD-HHMMSSmmm.log`. It shares the same HTTP ingestion component as `console` — **the push protocol is identical** (GET / POST / JSON / form / plain text, see [console](#console-box-webview-debug-log-ingestion) above); only the listening port and dump prefix differ:

```bash
tape app                              # listens on 0.0.0.0:8900; writes ./logs/app-YYYYMMDD-HHMMSSmmm.log
...
- **Instant console log dumps** ✅ done: pushes are written immediately to `console-YYYYMMDD-HHMMSSmmm.log`.
- **Box app network logs** ✅ done (`tape app`): boxes that cannot use logcat push their in-app logs / network events to tape via GET / POST, dumped as `app-YYYYMMDD-HHMMSSmmm.log`.
...
- Request and response bodies are fully buffered in memory before forwarding / saving (fine for APIs and static assets; there is no size cap, so very large or streaming bodies consume more memory — streaming is not implemented yet).
...
- record / replay / console / app are **unauthenticated** HTTP services that listen on all interfaces by default, so any LAN device can reach them (a deliberate debugging-tool design); use them on trusted networks, or restrict access with `--bind 127.0.0.1`.
