...
//! `{log-dir}/console-YYYYMMDD-HHMMSS.log`。
//!
//! 协议（详见 README「console 日志接收」）：
...
impl ConsoleState {
    fn new(log_dir: &std::path::Path, color: bool) -> Result<Arc<Self>> {
        std::fs::create_dir_all(log_dir)
            .with_context(|| format!("无法创建日志目录 {}", log_dir.display()))?;
        let log_path = log_file::path(log_dir, "console");
...
/// `tape console` 主流程：启动 HTTP 服务，Ctrl-C 优雅停止。
pub async fn run(args: ConsoleArgs) -> Result<()> {
    ...
    info!(
        "tape console 已启动: 0.0.0.0:{} （接收 GET/POST 调试日志，落盘 {}）",
        args.port,
...
async fn accept_loop(listener: TcpListener, state: Arc<ConsoleState>) -> Result<()> {
    loop {
        tokio::select! {
...
async fn serve_conn(state: Arc<ConsoleState>, stream: TcpStream, _addr: SocketAddr) -> Result<()> {
...
async fn handle_request(state: Arc<ConsoleState>, req: Request<Incoming>) -> Response<Full<Bytes>> {
    match *req.method() {
...
/// 打印 + 落盘一批日志，返回条数。
fn write_entries(state: &ConsoleState, entries: &[RawEntry]) -> usize {
    let mut stdout = std::io::stdout().lock();
    let mut file = state.file.lock().expect("console file lock poisoned");
    let mut n = 0;
