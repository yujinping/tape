...
> 专网 HTTP 接口录制与离线重放代理工具（纯 Rust）——首个正式发布。

tape 在「公司专网 / 有网环境」下把 App 的全部 HTTP 请求与响应录制为快照并下载静态资源，在「居家 / 无网环境」下以同一地址离线重放，使 App 在零外网、零专网请求的情况下完整复现界面与接口行为。
...
- **跨平台**：Windows / macOS / Linux 单二进制；`build.sh` 一键构建，CI 三平台自动测试与构建。
...
- 一键构建：`./build.sh`（当前系统）、`./build.sh mac`（macOS）、`./build.sh win`（Windows x64 交叉编译）
- 打 tag 后 CI 自动构建三平台 release 产物（见 Actions 运行记录）

## 已知限制

- App ↔ tape 之间仅 HTTP/1.1 明文，不做 HTTPS 服务端（App 无需装证书）；不支持 CONNECT 隧道 / MITM 抓 https 明文。
...
- [样例配置文件](../../tape-config.example.toml)
